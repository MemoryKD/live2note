...
def run_ytdlp(
    ...
    """Run yt-dlp -j and return parsed JSON. Raises YtdlpError on failure."""
    cmd = [ytdlp_path, "--no-download", "--no-warnings", "-j"]
    if cookie_file:
    ...
    try:
        ...
        raise YtdlpError("yt-dlp not found. Install it: pip install yt-dlp") from err
    except subprocess.TimeoutExpired as err:
        raise YtdlpError(f"yt-dlp timed out after {timeout}s") from err

    if result.returncode != 0:
        stderr = result.stderr.strip()
        raise YtdlpError(stderr or f"exit code {result.returncode}")

    stdout = result.stdout.strip()
    if not stdout:
        raise YtdlpError("Empty output from yt-dlp")
